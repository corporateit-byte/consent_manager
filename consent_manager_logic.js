...
class SilktideConsentManager {
  constructor(config) {
    ...
    // If enabled and no choice has been made yet, record each type's
    // defaultValue as the initial consent so it takes effect (and shows as
    // checked) immediately, while the banner is still shown for the user to
    // confirm or reject. This does NOT mark hasConsented - the banner always
    // shows on first visit regardless of this setting.
    if (this.config.preselectDefaults && !this.getHasConsented()) {
      this.config.consentTypes.forEach((type) => {
        if (this.getConsentChoice(type.id) === null) {
          this.setConsentChoice(type.id, type.required || !!type.defaultValue);
        }
      });
    }
  ...
  createCookieIcon() {
    ...
    this.icon.title = this.config.text?.icon?.title || 'Manage your consent preferences for this site';
